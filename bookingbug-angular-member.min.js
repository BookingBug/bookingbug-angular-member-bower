...
element.find("iframe").bind("load",function(_this){return function(event){var origin,url;return scope.wallet_payment_url&&(url=scope.wallet_payment_url),origin=getHost(url),sendLoadEvent(element,origin,scope),scope.$apply(function(){return scope.callSetLoaded()})}}(this))):void 0}),$window.addEventListener("message",function(_this){return function(event){var data;return angular.isObject(event.data)?data=event.data:event.data.match(/iFrameSizer/)||(data=JSON.parse(event.data)),scope.$apply(function(){if(data)switch(data.type){case"submitting":return scope.callNotLoaded();case"error":return scope.callSetLoaded(),scope.error(data.message);case"wallet_payment_complete":return scope.walletPaymentDone();case"basket_wallet_payment_complete":return scope.callSetLoaded(),scope.basketWalletPaymentDone()}})}}(this),!1)},{restrict:"A",link:link,controller:"Wallet",scope:!0,replace:!0}})}.call(this);